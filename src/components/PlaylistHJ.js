...
const PlaylistHJ = () => {
  ...
  useEffect(() => {
    // 음악 정보 가져오기
    axios
      .get(`https://music-json.run.goorm.site/playlistHJ`)
      .then((response) => {
        setPlaylistHJ(response.data);
      })
      .catch((error) => {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await axios.delete(
        `https://music-json.run.goorm.site/playlistHJ/${id}`
      );
      setPlaylistHJ(playlistHJ.filter((p) => p.id !== id));
